...
the_runtime_image_installs_ffmpeg_test() ->
    ...
    [_Builder, Runtime] = binary:split(Text, <<"FROM ghcr.io/macula-io/macula-pq-runtime">>),
    ?assertMatch({match, _},
                 re:run(Runtime, <<"apt-get install -y --no-install-recommends \\\\\n\\s+ffmpeg">>)).
...
the_claim_names_the_service_and_its_box_test() ->
    ...
    ?assertMatch({match, _}, re:run(Text, <<"- MCL_SERVICE_NAME=mcl-tube\\n">>)),
